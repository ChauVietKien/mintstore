...
// Cấu hình Cloudinary từ chuỗi URL chuẩn
...
export async function uploadImageToCloudinary(file: File, cloudName?: string): Promise<string> {
  ...
  try {
    ...
    console.error('Lỗi Upload ảnh lên Cloudinary:', errorDetails);
    throw new Error('Upload ảnh thất bại: ' + (typeof errorDetails === 'string' ? errorDetails : JSON.stringify(errorDetails)));
  }
}
